...
def generate_graphs_depiction(route_graphs, depot='Kraków'):
    ...
    colors = ['red', 'blue', 'green', 'yellow', 'purple']  # Extend this list for more vehicles
    legend_elements = []
    ...
    for i, RG in enumerate(route_graphs):
        ...
        node_labels = {node: f"{node} (Stop: {RG.nodes[node]['stop']})" if node != depot else node
                       for node in RG.nodes}

        # Draw nodes and edges for each vehicle
        nx.draw(RG, pos, labels=node_labels, with_labels=True, node_color='skyblue', 
                node_size=500, edge_color=colors[i % len(colors)], width=2, alpha=0.6)

        # Annotate edges with weights, formatted to two decimal places
        edge_labels = {edge: f"{weight:.2f}" for edge, weight in nx.get_edge_attributes(RG, 'weight').items()}
        nx.draw_networkx_edge_labels(RG, pos, edge_labels=edge_labels, font_size=7)

        # Add legend element for this vehicle
        legend_elements.append(Line2D([0], [0], marker='o', color='w', label=f'Vehicle {i+1}',
